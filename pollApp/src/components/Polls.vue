...
const polls = ref([]);
const question = ref("");
const option1 = ref({ caption: "" , presentationOrder: 0});
const option2 = ref({ caption: "" , presentationOrder: 1});
const voteOptions = ref([]);
const counter = ref(0);
var allOptions = [];

const addOption = () => {
  voteOptions.value.push({ caption: "" , presentationOrder: voteOptions.value.length + 2});
};
...
const getPolls = async () => {
  try {
    ...
    console.log("polls: ", polls.value[0].voteOptions)
  } catch (error) {}
};
